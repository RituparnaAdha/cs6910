...
Original file is located at
    https://colab.research.google.com/drive/1GRVwt2967MEOCDglqkke-nzl7CciJjkR
...
Run the cell for packages and helper function
"""
...
import functools
def encoding_(text):
    #decoder_target_data = np.zeros((len(text), rnn.max_decoder_seq_length, rnn.num_decoder_tokens), dtype="float32")
    encoder_input_data = np.zeros((1, 20), dtype="float32")
    #print(encoder_input_data.shape)
    #decoder_input_data = np.zeros((len(text), rnn.max_decoder_seq_length), dtype="float32")

    #for i, input in enumerate(text):
    for t, char in enumerate(text):
        encoder_input_data[0, t] = rnn.input_token_index[char]
    encoder_input_data[0, t + 1 :] =  rnn.input_token_index[" "]
...
"""*   Run the below cell then a text box appears
*   Fill it in with a english word of yours choice
*   Move to next cell 

"""

a= widgets.Text( disabled=False)
widgets.HBox([widgets.Label('English Text'), a])

"""

*   Running the below cell with provide you with the translated hindi word.
*   Each character of the word is represented as button
*   clicking on each button shows you its dependency on input characters
*   red indicates most relation and blue indicates least as shown below

![colorbar.png](data:image/png;base64,)
...
input_seq = encoding_(a.value)
decoded_sentence, attention_weights = decode_sequence(np.tile(input_seq ,(256,1)))
decoded_sentence = decoded_sentence[:-1]


items =[widgets.Button(description=d,layout=widgets.Layout(width='30px', height='30px')) for d in decoded_sentence]
output = widgets.Output()
display(widgets.HBox(items),output)

def visualize(attention_matrix, index):
  imp_array =[]
  for i in range(len(a.value)):
      imp_array.append([a.value[i],get_clr(attention_matrix[index][0][0][i])])
  return print_color(imp_array)

def on_button_clicked(b,index):
    visualize(attention_weights, index)

for i in range(len(items)):
    items[i].on_click(functools.partial(on_button_clicked, index=i))